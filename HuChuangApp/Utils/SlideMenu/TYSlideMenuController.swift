...
extension TYSlideMenuController: UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
        ...
        if let ctrl = viewController as? HCSlideItemController, let nowIndex = menuCtrls.firstIndex(of: ctrl) {
            if nowIndex - 1 < 0 || menuCtrls.count == 0 {
                return nil
            }
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerAfter viewController: UIViewController) -> UIViewController? {
        ...
        if let ctrl = viewController as? HCSlideItemController, let nowIndex = menuCtrls.firstIndex(of: ctrl) {
            if nowIndex + 1 > menuCtrls.count || menuCtrls.count == 0 {
                return nil
            }
            
            if nowIndex == menuCtrls.count - 1 {
                return nil
            }else {
                return menuCtrls[nowIndex + 1]
            }
    ...
    func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        ...
        let currentCtrl = pageViewController.viewControllers?.first
        if finished, let ctrl = currentCtrl as? HCSlideItemController, let nowIndex = menuCtrls.firstIndex(of: ctrl) {
...
class TYSlideCell: UICollectionViewCell {
    ...
    public var model: TYSlideItemModel! {
        didSet {
            ...
            titleLabel.font = model.isSelected ? model.selectedTextFont : model.textFont
    ...
    override func layoutSubviews() {
        ...
        bottomLine.frame = .init(x: (width - width * 0.5) * 0.5, y: height - 2, width: width * 0.5, height: 2)
    }
}
